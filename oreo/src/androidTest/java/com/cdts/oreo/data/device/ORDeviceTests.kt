...
class ORDeviceTests: BaseTestCase() {
    ...
    @Test
    fun testDevice() {
        ORDevice.androidAdId().subscribe({ signal() }, { signal() })
        await()
        print(ORDevice.androidID)
        print(ORDevice.pseudoUniqueID)
        print(ORDevice.ua)
        print(ORDevice.gsfid)
        ORDevice.googleEmailAccounts().subscribe({ signal() }, { signal() })
        await()
        print(ORDevice.buildSDKVersion)
        print(ORDevice.sensors)
        ORDevice.isFingerprintSensorPresent().subscribe({ signal() }, { signal() })
        await()
        ORDevice.areFingerprintsEnrolled().subscribe({ signal() }, { signal() })
        await()
        ...
        ORDevice.isNetworkAvailable().subscribe({ signal() }, { signal() })
        await()
        print(ORDevice.isWifiEnabled)
        print(ORDevice.ipv4Address)
        print(ORDevice.ipv6Address)
        ORDevice.ipv4OutsideAddress().subscribe({ signal() }, { signal() })
        await()
        ORDevice.wifiSSID().subscribe({ signal() }, { signal() })
        await()
        ORDevice.wifiLinkSpeed().subscribe({ signal() }, { signal() })
        await()
        ORDevice.wifiBSSID().subscribe({ signal() }, { signal() })
        await()
        ORDevice.wifiMAC().subscribe({ signal() }, { signal() })
        await()
        print(ORDevice.connectedWifiMacAddress)
        ORDevice.networkType().subscribe({ signal() }, { signal() })
        await()
        ...
        ORDevice.bluetoothMAC().subscribe({ signal() }, { signal() })
        await()
        ...
        ORDevice.imei().subscribe({ signal() }, { signal() })
        await()
        ...
        ORDevice.phoneNo().subscribe({ signal() }, { signal() })
        await()
        ...
        ORDevice.imsi().subscribe({ signal() }, { signal() })
        await()
        ORDevice.simSerial().subscribe({ signal() }, { signal() })
        await()
        print(ORDevice.simState)
        print(ORDevice.country)
        print(ORDevice.carrier)
        print(ORDevice.isSimNetworkLocked)
        ORDevice.activeMultiSimInfo().subscribe({ signal() }, { signal() })
        await()
        ORDevice.isMultiSim().subscribe({ signal() }, { signal() })
        await()
        ORDevice.numberOfActiveSim().subscribe({ signal() }, { signal() })
        await()
        ORDevice.activeSimInfo().subscribe({ signal() }, { signal() })
        await()
        ORDevice.lineNumber().subscribe({ signal() }, { signal() })
        await()
